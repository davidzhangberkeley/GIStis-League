# GIStis-League
Open Project, Spring 2026

IMPORTANT: 
DO NOT make any direct pushes to main. Only make pushes to your own branch. 

To do a task, follow these steps:
1. Retrieve the main code locally & create a new branch for your work
```bash
git checkout main
git pull
git checkout -b <YOUR_BRANCH_NAME>
```
2. Code up your feature/changes
3. Save & push your changes onto Github (this will keep your changes in your own branch)
```bash
git add .
git commit -m "<YOUR_COMMIT_MESSAGE_HERE"
git push origin <YOUR_BRANCH_NAME>
```
4. Go to pull requests in github
5. Select "New Pull Request"
6. Select the main branch and your branch (the branch with <YOUR_NAME_HERE>)
7. Create the new pull request
8. Let David and Emily know
9. The PR (pull request) will be verified/denied by David and Emily. 

To set up your repository intially (ONLY DO THIS ONCE): 
```bash
git clone https://github.com/davidzhangberkeley/GIStis-League.git
```

To set up the Conda environment on VSCode (ONLY DO THIS ONCE):
1. go to the project directory on VSCode
2. press CMD + Shift + P
3. navigate to "Select Python Interpreter" and select Python /opt/anaconda3/envs/gistice_leage/python
